...
import { RegionTab, Region, SortOrder, Category } from '@/lib/types';
import { categoryOptions, sortOptions, countriesByRegion, CountryFlag } from '@/lib/constants';
...
export default function MobileFilterRow() {
  ...
  const currentCategory = categoryOptions.find((c) => c.id === activeCategory);
            ...
            {categoryOptions.map((opt) => (
              <button
                ...
                className={cn(
                  ...
                  activeCategory === opt.id ? 'text-[#3B82F6] bg-[#3B82F6]/10' : 'text-white hover:bg-white/5'
                )}
              >
                {opt.label}
              </button>
